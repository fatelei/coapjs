...
var Cache = require('memory-storage');
    ...
    if (handler === null) {
      ...
      try {
        if (self.middleware.hasOwnProperty('cache')) {
          if (req.method === 'GET') {
            result = self.middleware.cache.get(req.path);
          }
        }

        if (result === null) {
          // cache miss
          handler(req, res, function(info) {
            // whether the response can be cached
            if (res.getStatusCode() === '2.05') {
              if (self.middleware.hasOwnProperty('cache')) {
                var age = res.getHeader('Max-Age');
                age = age === null ? 60 : age;
                self.middleware.cache.setex(req.path, age * 1000, info.payload);
              }
            }
            self.message.send(info, self.isBackOff, client);
          });
        } else {
          var resInfo = res.end(result.toString());
          self.message.send(resInfo, self.isBackOff, client);
        }
      } catch(err) {
        // send 5.xx
        res.setHeader('Content-Format', 'text/plain;charset=utf-8');
