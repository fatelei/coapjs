...
function CoAPClient (port, host, type) {
  ...
  this.socket.on("message", function (msg, rinfo) {
    ...
    unpackMsg.payload = unpackMsg.payload.toString();
...
// set options
CoAPClient.prototype.setOptions = function (options) {
  var that = this;

  for (var i = 0; i < options.length; i++) {
    var keys = Object.keys(options[i]);
    var value = options[i][keys[0]];

    if (typeof value === 'number') {
      value = value.toString();
    }

    that.packet.options.push({"name": keys[0], "value": new Buffer(value)});
  }
};
...
CoAPClient.prototype.sendConfirm = function (code) {
  ...
  if (typeof code === 'undefined') {
    ...
    if (code === "0.00") {
      ...
      that.packet.code = code;
      var tmp = that.nextToken();
      that.packet.token = new Buffer(tmp.toString());
    }
  }
