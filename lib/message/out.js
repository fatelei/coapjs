...
OutcomeMessage.prototype.setCode = function () {
  ...
  var method_codes = macro.method_codes;

  var code = that.packet.code.toLowerCase();

  // code can be humized or not
  if (method_codes.hasOwnProperty(code)) {
    return method_codes[code];
  } else {
    var split = code.split(".").map(function (element) {
